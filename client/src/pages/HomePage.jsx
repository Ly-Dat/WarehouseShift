...
function getRelevantRows(allData, weekStartDate) {
  ...
  const latestTs = Math.max(
    ...inWeek.map((r) => parseDate(r.applyDate).getTime()),
  );
...
function autoSelectOptimal({
  ...
  function weekCount(emp) {
    ...
    for (const s of SHIFTS)
      for (let d = 0; d < 7; d++) if (result[`${emp}|${s}|${d}`]) c++;
    return c;
  }
...
function AutoSelectModal({
  ...
  function handlePreview() {
    const result = autoSelectOptimal({
      allData,
      employees,
      weekStart,
      weekKey,
    ...
    >
      ...
      >
          ...
          >
            <div>
              ...
              <div style={{ fontSize: 12, color: "#6B7280", marginTop: 4 }}>
                Nhập số người mỗi ca/ngày — thuật toán phân ca công bằng tối ưu
              </div>
            </div>
            <button
              onClick={onClose}
        ...
        {hasPreview && (
          ...
          >
            {[
              { bg: "#D1FAE5", color: "#059669", label: "Đủ người ✓" },
              { bg: "#DBEAFE", color: "#2563EB", label: "Dư người" },
              { bg: "#FEE2E2", color: "#DC2626", label: "Thiếu người" },
            ].map((l) => (
              <div
                key={l.label}
                style={{
        ...
        >
          <strong>Thuật toán công bằng:</strong>
          <br />• <strong>Thêm ca:</strong> ưu tiên nhân viên ít ca nhất tuần
          này → ít ca nhất lịch sử
          <br />• <strong>Cắt ca:</strong> ưu tiên cắt nhân viên nhiều ca nhất
          tuần này → nhiều ca nhất lịch sử
          <br />• Nếu thiếu người do không đủ nhân viên rảnh, số hiển thị sẽ nhỏ
          hơn target
        </div>
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const data =
          json && Array.isArray(json.assignments) ? json.assignments : [];
  ...
  const savedWeekAssignments = buildWeekAssignments(
    savedAllRef.current,
    weekKey,
  );
  ...
  const hasChanges =
    stableKeys(weekAssignments) !== stableKeys(savedWeekAssignments);
  ...
  const assignedEmps = new Set(
    Object.keys(weekAssignments)
      .filter((k) => weekAssignments[k])
      .map((k) => k.split("|")[0]),
  ).size;
  ...
  function handleAutoApply(newWeekAssignments) {
    setAllAssignments((prev) =>
      mergeIntoAll(prev, weekKey, newWeekAssignments),
    );
    setSaveStatus("idle");
  }
  ...
  async function handleSave() {
    if (!SCRIPT_URL) {
      alert("VITE_SCRIPT_URL chưa được thiết lập trong .env");
      return;
    }
    ...
    try {
      const payload = {
        weekStart: weekKey,
        assignments: assignmentsToArray(weekAssignments),
      };
      ...
      if (json.ok) {
        savedAllRef.current = mergeIntoAll(
          savedAllRef.current,
          weekKey,
          weekAssignments,
        );
        try {
          localStorage.setItem(EMP_ORDER_KEY, JSON.stringify(employees));
        } catch {}
        setSaveStatus("saved");
  ...
  const effectiveStatus =
    saveStatus === "idle" && hasChanges ? "unsaved" : saveStatus;
  const saveBtnStyle = {
          ...
          {!isPersonalMode && (
            <button
              ...
              style={{
                ...
                transition: "all 0.2s",
                opacity:
                  saveStatus === "saving" || loadingAssignments ? 0.7 : 1,
                display: "flex",
                alignItems: "center",
                gap: 6,
              }}
              ...
              >
                <div
                  style={{
                    ...
                    textTransform: "uppercase",
                    letterSpacing: "0.05em",
                    color: i === todayIdx ? "#2563EB" : "#9CA3AF",
                  }}
                >
                  {DAYS_EN[i]}
                </div>
                <div
                  style={{
                    fontSize: 18,
                    fontWeight: 500,
                    color: i === todayIdx ? "#2563EB" : "#111827",
                    marginTop: 2,
                  }}
                >
          ...
          {SHIFTS.map((shift, si) => (
            <div key={shift}>
              ...
              >
                ...
                {[0, 1, 2, 3, 4, 5, 6].map((di) => {
                  const count = employees.filter(
                    (emp) => weekAssignments[`${emp}|${shift}|${di}`],
                  ).length;
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        textAlign: "center",
                        borderRight:
                          di < 6 ? "0.5px solid rgba(0,0,0,0.06)" : "none",
                        display: "flex",
                        alignItems: "center",
                        justifyContent: "center",
                      }}
                    >
                      <span
                        style={{
                          ...
                          borderRadius: 11,
                          background:
                            count > 0 ? SHIFT_STYLES[si].text : "transparent",
                          color: count > 0 ? "#fff" : SHIFT_STYLES[si].text,
                          border:
                            count > 0
                              ? "none"
                              : `1.5px dashed ${SHIFT_STYLES[si].text}`,
                          fontSize: 11,
                          fontWeight: 700,
                          display: "flex",
                          alignItems: "center",
              ...
              {employees.map((emp, ei) => (
                ...
                >
                  <div
                    ...
                    onDragOver={
                      si === 0 ? (e) => e.preventDefault() : undefined
                    }
                    onDrop={
                      si === 0
                        ? (e) => {
                            e.preventDefault();
                            moveEmployee(dragRef.current.dragIdx, ei);
                    ...
                    onTouchStart={
                      si === 0
                        ? (e) => {
                            ...
                            const rows =
                              document.querySelectorAll("[data-emp-row]");
                            dragRef.current.rowTops = Array.from(rows).map(
                  ...
                  >
                    ...
                    {(() => {
                      ...
                      const available = countAvailableShifts(
                        emp,
                        allData,
                        weekStart,
                      );
